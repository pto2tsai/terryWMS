...
- 鼎新報表自動匯入（設定步驟見 `docs/ERP報表自動匯入設定.md`）：鼎新定時匯出到 Google 雲端硬碟 → Google 雲端自動程式（`tools/erp-sync`）每 10 分鐘把新檔整份存進 `erpInbox` →
  WMS 電腦版開著就自動接手「每日客戶銷貨明細表」：匯入訂單、依物流商建波次（不跳視窗；件數換算不出來的單留給「手動匯入」）；其他報表存檔，在「ERP 報表（鼎新）」頁檢視、下載。
  應收帳款、月銷貨等含金額的報表只有主管／財務／管理員看得到；看板與首頁會提醒要處理的匯入
...
| 23-erp-inbox | 鼎新報表收件匣：自動匯入訂單（`autoImportErpOrderRows`）、多台電腦只由一台處理（交易認領）、ERP 報表頁（檢視、下載、手動匯入） |
