    ...
    try {
        ...
        for (var i = 0; i < docs.length; i++) {
            ...
            try {
                ...
                upd = { status: out.issues.length ? 'attention' : 'done', result: out.result, issues: out.issues };
            } catch (e) {
    ...
    tbody.innerHTML = rows.map(function(r) {
        ...
        return '<tr class="border-b border-slate-700/50 align-top">' +
            ...
            '<td class="p-2 whitespace-nowrap">' +
                ...
                (canManual ? '<button onclick="manualImportErp(\'' + r.id + '\')" class="px-2 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded text-xs">手動匯入</button>' : '') +
            '</td></tr>';
    }).join('');
};
