...
H.check('不同批號：擋下不能合併，兩板都保留（19 件不變）', log.dialogs.slice(n).some(d => /無法合併：批號不同/.test(d.msg)) && p1.quantity === 19 && !!(await H.one('pallets', 'IN-20260901-003')), JSON.stringify(p1));
// 備案查詢：用儲位找板
...
H.check('移動與合併都有記錄（move 1、merge 1）', lg.filter(l => l.type === 'move').length === 1 && lg.filter(l => l.type === 'merge').length === 1, JSON.stringify(lg.map(l => l.type)));
