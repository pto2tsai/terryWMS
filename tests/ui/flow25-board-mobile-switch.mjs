...
await H.resetData(async d => { await baseSeed(d);
  ...
  await H.setDoc(H.doc(d, 'waves', 'W1'), { waveNo: 'W260924-001', logistics: '黑貓', status: 'picking', totalQty: 20, orderCount: 3, itemCount: 4, completedItems: ['a', 'b'], createdAt: new Date(Date.now() - 45 * 60000).toISOString() });
...
H.check('像出餐螢幕：波次顯示等了多久（45 分），超過 30 分鐘變黃', (await txt('l-waves')).includes('45 分') && (await B.$$('#l-waves .card.warn')).length === 1, await txt('l-waves'));
...
await H.admin(async d => { await H.setDoc(H.doc(d, 'dispatchOrders', 'D1'), { orderNo: 'DSP-1', productName: '白蝦', status: 'pending', operations: [{ id: 'op-0' }, { id: 'op-1' }], completedOps: [] });
  await H.setDoc(H.doc(d, 'waves', 'W0'), { waveNo: 'W260924-000', logistics: '新竹', status: 'pending', totalQty: 8, orderCount: 1, itemCount: 2, completedItems: [], createdAt: new Date(Date.now() - 90 * 60000).toISOString() });
...
const wv = await B.evaluate(() => [...document.querySelectorAll('#l-waves .card')].map(c => ({ t: c.innerText.split('\n')[0], late: c.classList.contains('late'), fresh: c.classList.contains('fresh') })));
H.check('新進來的波次閃一下；等超過 60 分鐘變紅；揀貨中的仍排最上面', wv.length === 2 && wv[0].t.includes('W260924-001') && wv[1].late && wv[1].fresh, JSON.stringify(wv));
