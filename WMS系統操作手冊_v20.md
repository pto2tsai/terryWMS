...
板型依填充率判斷：≥80% 整板、50%~79% 不足板、<50% 散板。
同一層混放不同板型時依比例計算，例如 2F 已放 2 整板（佔 2/4），還能再放 3 散板（3/6）。
...
**網址：** 電腦版網址後面加 `/m/`（例如 `https://你的網址/m/`）。舊的 `mobile.html` 會自動轉過去。
用 Chrome／Safari 開啟後選「加到主畫面」，之後就像 App 一樣從桌面開。帳號與電腦版相同。

### 9A.1 功能

| 功能 | 用途 | 操作 |
|---|---|---|
| 波次揀貨 | 訂單出貨 | 選波次 → 連續掃板號（或儲位標籤）打勾 → 完成波次（扣庫存、訂單出貨） |
| 入庫任務 | 電腦建立的入庫單 | 點任務或掃棧板單 → 放上儲位 → 掃儲位標籤 → **直接入帳** |
| 調度工單 | 電腦「發布到手機」的移板／併板 | 選工單 → 連續掃板號，掃到就執行 → 完成工單 |
| 盤點 | 逐板點數 | 掃板號或儲位 → 輸入實際數量（或按「與帳面相同」）→ 有差異才調整 |
| 上架 | 把棧板放到儲位 | 掃儲位 → 掃棧板 → 確認件數（點數不同可直接改） |
| 出庫 | 直接從某一板扣數量 | 掃棧板 → 輸入數量（+1/+5/全部）→ 確認 |
| 移板 | 換儲位 | 掃棧板 → 掃新儲位 |
| 併板 | 兩板併一板 | 掃要併掉的板 → 掃要併入的板 → 確認 |
| 庫存快查 | 查品項在哪 | 打品名、板號、批號，或掃儲位標籤看該儲位有什麼 |

主選單上的紅色數字是待處理數量，有新的工作會跳通知並震動。

### 9A.2 掃描

- 每個掃描欄旁的綠色 📷 開啟相機，對準棧板單或儲位標籤的 QR code（CODE128 條碼也可以）
- **揀貨、調度是連續掃描**：掃到一板就打勾，相機不關，直接換下一板
- 有下一步要掃的（例如移板：先掃板、再掃新儲位），相機會自動換到下一格，不用關掉再開
- 要輸入數量時相機會自動關閉
- 光線不足按「補光」開手電筒（Android 大多支援）
- 標籤掃不到時手打**板號尾碼**即可（至少 3 碼，例如 `0015`）；尾碼相同的有兩板以上會列出讓你點選
- 儲位可以打縮寫：`IA011` = `I-A-01-1F`
- 需要掃「棧板」的地方也可以掃**儲位標籤**：該儲位只有一板就直接帶出，有多板會列出讓你選
- 不分大小寫、`-` 可省略；藍牙掃描槍也可以用（掃描槍結尾送 Enter 即可，多數預設就是）
- 第一次使用會詢問相機權限，請按「允許」

### 9A.3 規則（和電腦版一樣）

- 所有庫存變動都走交易：兩個人同時操作同一板不會互相蓋掉；庫存不足會擋下
- 入庫放的儲位和指定的不同 → 先確認，以**實際儲位**入帳
- 入庫單尚待核准 → 提醒但可以先上架
- 併板批號／效期不同 → 提醒，可以併（效期取較早的）；品名／規格／公司不同 → 不能併
- 盤點期間該板被別人異動過 → 不覆蓋，請重新掃描該板
- 每一筆都會留下異動記錄（含操作人）
