...
2. 系統自動配對庫存
3. 產生揀貨清單
4. 掃描確認揀貨（手機可用相機掃，見下方「手機掃碼」）
5. 完成出庫

**手機掃碼：**
- 掃描欄旁的綠色 📷 按鈕開啟相機，對準棧板單或儲位標籤上的 QR code（條碼 CODE128 也可以）
- 揀貨、調度是**連續掃描**：掃到一板就打勾，相機不關，直接換下一板；按「完成」關閉
- 入庫確認、庫存快查掃到就自動關閉
- 光線不足時按「補光」開手電筒（Android 大多支援；iPhone 瀏覽器不支援時不會顯示按鈕）
- 標籤髒污掃不到時，手打**板號尾碼**即可（至少 3 碼，例如打 `0015`）；有兩板以上尾碼相同會提示多打幾碼
- 不分大小寫；第一次使用瀏覽器會詢問相機權限，請按「允許」
